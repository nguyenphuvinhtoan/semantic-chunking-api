...
class SemanticChunker(BaseDocumentTransformer):
    ...
    def split_text_codelight(
        ...
        # having len(grouped_sentences) == 1 would cause the following
        # np.percentile to fail.
    ...
    def create_documents_codelight(
        self, texts: List[str], metadatas: Optional[List[dict]] = None, sentences_per_group: int = 1
    ) -> List[Document]:
        """Create documents from a list of texts."""
        documents = []
        for text in texts:
            start_index = 0
            chunks = self.split_text_codelight(text, sentences_per_group)
            total_chunks = len(chunks)
            
            # Generate all UUIDs first to maintain prev/next relationships
